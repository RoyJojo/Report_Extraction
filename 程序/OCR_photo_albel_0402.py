...
def download_file_from_url(url, temp_dir):
    ...
    try:
        ...
        response = requests.get(
            url,
            stream=True,
            headers=headers,  # 关键：模拟浏览器请求
            timeout=(10, 30),  # 10秒连接超时，30秒读取超时
            verify=False  # 临时关闭SSL验证（生产环境需配置证书）
        )
...
def upload_to_minio(file_path):
    try:
        # object_name = secure_filename(os.path.basename(file_path))
...
def analyze_contract(image_url):
    messages = [
...
"notes": "Impurity result from random truck checking is referential only. Bridge scale type SCS-120, expiry 2026-07-08. Place of inspection: Shandong, China."
    ...
    try:
        ...
        print(f"[OpenAI错误] {e}")
        return Response(json.dumps({"code": 500, "msg": "解析失败", "error": f"[OpenAI错误] {e}"}, ensure_ascii=False),
                        status=200,
...
def mark_extracted_values(image_path, extracted_data, output_dir=None):
    ...
    参数:
        image_path: 原始图片路径
        extracted_data: 从合同提取的数据(dict)
        output_dir: 标注图片输出目录(可选)

    返回:
        tuple: (标注后图片路径, 找到的值和位置信息) 或 Flask错误响应
    """
...
def process_single_file(file_path, file_type, temp_dir):
    ...
    if file_type == 'pdf':
        ...
        for i, img_path in enumerate(images):
            ...
            if isinstance(extracted_data, Response):  # 如果是错误响应
    ...
    # Word 处理
        ...
        try:
            ...
            return Response(json.dumps({"code": 500, "msg": "解析失败", "error": f"[Word文件处理失败] {e}"}, ensure_ascii=False),
                            status=200,
                            content_type='application/json; charset=utf-8')
...
def extract_and_mark_api():
    ...
    try:
        ...
        print(f"[处理失败] {e}")
        return Response(json.dumps({"code": 500, "msg": "解析失败", "error": f"[处理失败] {e}"}, ensure_ascii=False),
                        status=200,
                        content_type='application/json; charset=utf-8'), 200
